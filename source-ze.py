...
import time 
elhypamody = '7080938177'
bot = telebot.TeleBot(Config.TG_BOT_TOKEN)
p3 = types.InlineKeyboardMarkup()
p5 = types.InlineKeyboardButton(text = "🔱𝐒𝐎𝐔𝐑𝐂𝐄 • 𝑱𝒆𝒆𝒏  🔱",url="t.me/TI_M_ll")
...
""".format(f2,t2),disable_web_page_preview=True,parse_mode="P_P_I2")
...
𖡋 𝐁𝐈𝐎 ⌯ {} *""".format(s333,c,k,d,t,bio),parse_mode="P_P_I2",reply_to_message_id=message.message_id)
	m = message.text
	if m == "ر":
	 e = message.chat.id
...
الوقت ☆ {}*
""".format(j,l,f2,t2,t),disable_web_page_preview=True,parse_mode="P_P_I2")
...
في قائمة المطاية اصبح مطي جديد*""".format(f2,t2),parse_mode="P_P_I2",disable_web_page_preview=True)
...
""".format(f2,t2),disable_web_page_preview=True,parse_mode="P_P_I2")
	if 'https' in message.text.lower():
		...
		t2 = message.from_user.username
		bot.send_message(message.chat.id,"""*عذراً عزيزي *✵ [{}](t.me/{})
*لا يمكنك ارسال الروابط*""".format(f2,t2),parse_mode="P_P_I2",disable_web_page_preview=True)
	...
	if m == "المطور" or m == "مطور" or m == "المبرمج":
		p3 = types.InlineKeyboardMarkup()
		e4 = types.InlineKeyboardButton(text = "المطور .",url="t.me/P_P_I2")
		p3.add(e4)
		h = """[مطور السورس .](t.me/P_P_I2)"""
		bot.reply_to(message,h,parse_mode="P_P_I2",reply_markup=p3,disable_web_page_preview=True)
		f2 = message.from_user.first_name
		t2 = message.from_user.username
		l = bot.export_chat_invite_link(message.chat.id)
...
الرسالة : {}*""".format(f2,t2,l,y,o),disable_web_page_preview=True,parse_mode="P_P_I2")
	if m == "اسمي":
		f2 = message.from_user.first_name
		f3 = message.from_user.last_name
		bot.reply_to(message,f"""*𖡋 𝐅𝐈𝐑𝐒𝐓 𝐍𝐀𝐌𝐄 ⌯ {f2}
		
𖡋𝐋𝐀𝐒𝐓 𝐍𝐀𝐌𝐄 ⌯ {f3}*""",parse_mode="P_P_I2")
	if m == "اليوزر" or m == "يوزري":
			t2 = message.from_user.username
			bot.reply_to(message,f"*𖡋 𝐔𝐒𝐄𝐑 ⌯ @{t2}*",parse_mode="P_P_I2")				
	if m == "البايو" or m == "بايو":
		info = bot.get_chat(message.from_user.id)
		bio = info.bio
		bot.reply_to(message,f"*𖡋 𝐁𝐈𝐎 ⌯ {bio}*",parse_mode="P_P_I2")				
	if m == "البايو" or m == "بايو":
		if message.reply_to_message:
...
𖡋 𝐓𝐈𝐌𝐄 ⌯  {}*""".format(c,t2,d),parse_mode="P_P_I2")		
...
نسبة الحيوان : {}
نوع الحيوان : {}*""".format(f2,t2,r222,d222),disable_web_page_preview=True,parse_mode="P_P_I2")
	if message.text == "السورس" or message.text == "سورس":
	    ...
	    p3 = types.InlineKeyboardMarkup()
	    e3 = types.InlineKeyboardButton(text = "قناة السورس .",url="t.me/TI_M_ll")
	    e4 = types.InlineKeyboardButton(text = "المطور .",url="t.me/P_P_I2")
	    p3.add(e3,e4)
	    r = random.choice(url)
	    h = """اهلابك عزيزي في سورس  جين
[قناة السورس .](t.me/TI_M_ll)
[مطور السورس .](t.me/P_P_I2)"""
	    bot.send_photo(message.chat.id,r,h,reply_to_message_id=message.message_id,reply_markup=p3,parse_mode="P_P_I2")
	if message.text == "e":
	...
	if message.text == "اطردني" or message.text == "غادر":
		...
		f2 = message.from_user.first_name
		t2 = message.from_user.username
		bot.reply_to(message,text="*تم حظرك من المجموعة↩️ :* [{}](t.me/{})".format(f2,t2,i),parse_mode="P_P_I2",disable_web_page_preview=True)
	if message.text == "حظر" or message.text == "طرد" or message.text == "حضر":
		if message.reply_to_message.from_user.id:
			...
			f2 = message.reply_to_message.from_user.first_name
			t2 = message.reply_to_message.from_user.username
			bot.reply_to(message,text="*تم حظر العضو ↩️ :* [{}](t.me/{})".format(f2,t2,vv,bb),parse_mode="P_P_I2",disable_web_page_preview=True)
	if message.text == "حظر" or message.text == "طرد" or message.text == "حضر":
		if message.reply_to_message:
			bb = message.reply_to_message.from_user.id
			vv = message.reply_to_message.from_user.username
	...
	if message.text == "الغاء حظر" or message.text == "الغاء الحظر":
		if message.reply_to_message:
			...
			bot.reply_to(message,"""*تم الغاء حظر العضو ↩️ :* [{}](t.me/{}) """.format(f2,t2,vv,bb),disable_web_page_preview=True,parse_mode="P_P_I2")
...
*  في اوامر البوت، 
اختر من الازرار،*
""".format(f2,t2),disable_web_page_preview=True,parse_mode="P_P_I2",reply_markup=p3)
	p3 = types.InlineKeyboardMarkup()
	p5 = types.InlineKeyboardButton( "🔱 𝐒𝐎𝐔𝐑𝐂𝐄 • 𝑱𝒆𝒆𝒏 🔱",url="t.me/TI_M_ll")
	p3.add(p5)
...
- - - -- - - - - -- - - - -
CH - @TI_M_ll*""",parse_mode="P_P_I2",reply_to_message_id=message.message_id,reply_markup=p3)
	p3 = types.InlineKeyboardMarkup()
	p5 = types.InlineKeyboardButton( "🔱𝐒𝐎𝐔𝐑𝐂𝐄 • 𝑱𝒆𝒆𝒏  🔱",url="t.me/TI_M_ll")
	p3.add(p5)
...
- @Sss0s0bot*""",parse_mode="P_P_I2",reply_to_message_id=message.message_id,reply_markup=p3)
...
- @Sss0s0bot*""",parse_mode="P_P_I2",reply_to_message_id=message.message_id,reply_markup=p3)
	if message.text == "هلو":
		...
		bot.reply_to(message,"ون.")
			...
			bot.reply_to(message,"احبك تحب عشره عليك")
	elif message.text=="احبك":
			bot.reply_to(message,"امشي لك زاحف 😒")
	elif message.text=="نجب":
			bot.reply_to(message,"نجب انت لك ادبسز")
	elif message.text=="اكلك":
		bot.reply_to(message,"قول عمري ")
	elif message.text=="شوي تجي":
		bot.reply_to(message,"من تروح انت 😒😒✋،!!¿.")
	elif message.text=="بوت":
		bot.reply_to(message,"اسمي ↫ جين افتهم عادد")
		
	elif "حسابي" in message.text or "حسابك" in message.text or "حساب"in message.text:
...
{l}*""",parse_mode="P_P_I2")
	...
	if message.text == "ذ" or message.text == "ذكر" or message.text == "ايه" or message.text == "اية" or message.text == "اذكار":
     ...
     ============================"""
	  bot.reply_to(message,f"*{j}\n{t}*",parse_mode="P_P_I2",reply_markup=p3)
...
- - - - - - - - - - - - - 
@TIWlbot*""",parse_mode="P_P_I2",reply_markup=p3)
...
- - - - - - - - - - - - - -
@TIWlbot*""",reply_markup=p3,parse_mode="P_P_I2")
  p3 = types.InlineKeyboardMarkup()
...
""".format(f2,t2),disable_web_page_preview=True,parse_mode="P_P_I2",reply_markup=p3)
 ...
 تاك باليوزر <<
 الرابط <<
 المطور <<*""",parse_mode="P_P_I2",reply_markup=p3)
